"""
experiment_num_polar.py — Hyperparameter sweep over OrthLayer num_polar (polar basis count).

Sweeps num_polar from 1 to 8 (3 runs each), evaluates every value on the
NUDT-SIRST test set, and writes a summary Excel file.

Usage:
    python experiment_num_polar.py [--epochs 400] [--batchSize 4] [--threads 1]
...
#  CLI
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="Sweep OrthLayer num_polar (polar basis count)")
...
parser.add_argument("--lr", type=float, default=5e-4,
                    help="Learning rate")
parser.add_argument("--save_dir", default="./experiment_logs/num_polar_sweep",
...
# ---------------------------------------------------------------------------
NUM_POLAR_VALUES = list(range(1, 9))    # 1 … 8
N_RUNS = 3
BASE_SEED = 42
OPTIMIZER_NAME = "Adam"
SCHEDULER_NAME = "MultiStepLR"
SCHEDULER_STEP = [opt.epochs // 2, int(opt.epochs * 0.75)]
...
def train_one_run(num_polar_value, run_idx, seed):
    """
    Train SFBD_Net with a specific num_polar on NUDT-SIRST for ``opt.epochs``
    epochs.  Returns the best metrics seen (after epoch > epochs/2).
    """
    ...
    # ---- Model ----
    net = Net(model_name="SFBD_Net", mode="train", num_polar=num_polar_value).cuda()
    net.train()
    ...
    for idx_epoch in range(opt.epochs):
        ...
        if idx_epoch + 1 > opt.epochs // 2:
            ...
            print(f"    [num_polar={num_polar_value} run={run_idx}] "
                  f"epoch {idx_epoch+1:3d}  loss={avg_loss:.4f}  "
                  f"mIoU={miou:.4f}  fscore={fscore:.4f}")

    return {
        "num_polar": num_polar_value,
        "run": run_idx,
        "seed": seed,
        "best_mIoU": best_miou,
        "best_PD": best_pd,
...
def main():
    ...
    print(f"{'='*60}")
    print(f"num_polar sweep: {NUM_POLAR_VALUES}  ×  {N_RUNS} runs  ×  {opt.epochs} epochs")
    print(f"Save dir: {opt.save_dir}")
    print(f"{'='*60}")

    for np_val in NUM_POLAR_VALUES:
        for run in range(1, N_RUNS + 1):
            seed = BASE_SEED + run * 100 + np_val
            seed_pytorch(seed)

            run_name = f"NP{np_val:02d}_run{run}"
            print(f"\n>>> {run_name}  (seed={seed})")

            t0 = time.time()
            result = train_one_run(np_val, run, seed)
...
def _write_excel(results):
    ...
    ws_raw = wb.active
    ws_raw.title = "raw"
    headers = ["num_polar", "run", "seed", "best_mIoU", "best_PD", "best_FA", "best_Fscore"]
    ws_raw.append(headers)
    for r in results:
        ws_raw.append([r[h] for h in headers])

    # ---- Sheet 2: aggregated (mean ± std per num_polar) ----
    ws_agg = wb.create_sheet("aggregated")
    agg_headers = ["num_polar", "mean_mIoU", "std_mIoU", "mean_PD", "std_PD",
                   "mean_FA", "std_FA", "mean_Fscore", "std_Fscore"]
    ws_agg.append(agg_headers)
    for np_val in NUM_POLAR_VALUES:
        group = [r for r in results if r["num_polar"] == np_val]
        row = [np_val]
    ...
    # ---- Sheet 3: best per num_polar ----
    ws_best = wb.create_sheet("best_per_num_polar")
    ws_best.append(["num_polar", "best_mIoU", "best_PD", "best_FA", "best_Fscore", "run"])
    for np_val in NUM_POLAR_VALUES:
        group = [r for r in results if r["num_polar"] == np_val]
        best = max(group, key=lambda x: x["best_mIoU"])
        ws_best.append([np_val, best["best_mIoU"], best["best_PD"],
                        best["best_FA"], best["best_Fscore"], best["run"]])

    path = os.path.join(opt.save_dir, "num_polar_sweep_results.xlsx")
    wb.save(path)
...
def _write_csv(results):
    """Fallback CSV output."""
    path = os.path.join(opt.save_dir, "num_polar_sweep_results.csv")
