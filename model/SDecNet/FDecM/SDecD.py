...
class SD2D(nn.Module):
    ...
    def Extract_layer(self,cen,b,w,h):
        ...
        max = self.max_pool(cen) - avg_cen
        avg_cen = avg_cen - torch.mean(avg_cen,dim=[2,3],keepdim=True)
        basis = torch.concat([max.view(b,self.hidden_channels,1,-1),avg_cen.view(b,self.hidden_channels,1,-1),edge],dim=2)
