...
class SS2D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        self.selective_scan = selective_scan_fn
        # selective_scan(xs, dts, As, Bs, Cs, Ds, z=None, delta_bias=None, delta_softplus=True, return_last_state=False)
        # selective_scan 是选择性扫描操作，
        B, C, H, W = x.shape
        L = H * W
        K = 4
        ...
        # 将图片进行翻转，也就是0,90,180,270,四个方向进行提取
        # batch数， 采样方向数， 通道数， H*W
        x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs.view(B, K, -1, L), self.x_proj_weight)
        # 进行线性投影方法，相当于在各个方向上的偷盗进行conv1*1
        ...
        # K代表方向
        # d代表通道
        # 将四个不同扫描方向的时间参数拼接成一个连续张量，让GPU能够一次性处理所有方向的时许依赖
        Bs = Bs.float().view(B, K, -1, L) # (b, k, d_state, l)
        Cs = Cs.float().view(B, K, -1, L) # (b, k, d_state, l)
        # 保持每个扫描方向独立的输入输出投影矩阵，确保不同方向的扫描过程具有不同的动态特性，让模型能够学习到方向相关的特征变换模式。
        Ds = self.Ds.float().view(-1) # (k * d)
        # 将跳跃连接参数压缩成一维向量，便于在选择性扫描函数中执行高效的逐元素加法操作，实现残差连接的数学表达 y = C*h + D*x。
        As = -torch.exp(self.A_logs.float()).view(-1, self.d_state)  # (k * d, d_state)
        # 强制状态转移矩阵A的对角元素为正数，保证转该矩阵的稳定性，防止隐藏状态发散
